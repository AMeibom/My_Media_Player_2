package com.example.my_media_player_2;
...
public class MainActivity extends AppCompatActivity implements Runnable{
    ...
    private FloatingActionButton fabForward;// поле кнопки перемотки назад аудиофайла
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        audioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);//Извлекаеи системную службу Audio Manager
        int maxVolume = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);//Максимальный уровень громкости
        int curValue = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);//Минимальный уровень громкости

        volumeControl.setMax(maxVolume); //Максимальное значение на панель поиска
        volumeControl.setProgress(curValue);// Текущие значение на панели поиска
        ...
        fabForward.setOnClickListener(listener);// создание слушателя нажатия кнопки fabBack (перемотки вперед)
    ...
    public void playSong() {
        try { // обработка исключения на случай отстутствия файла
            ...
            if (!wasPlaying) {
                ...
                metaData = mediaMetadata.extractMetadata(MediaMetadataRetriever.METADATA_KEY_ARTIST); // добавление названия трека
                metaData += "\n" + mediaMetadata.extractMetadata(MediaMetadataRetriever.METADATA_KEY_TITLE); // добавление артиста
